    ...
    return (
        <div className="flex flex-wrap justify-center">
            {Object.keys(photos).map((album, index: number) => {
                if (photos[album].photos.length === 0) {
                    return <></>;
                }
                return (
                    <div key={index} className="w-80 h-112 flex flex-col items-center justify-center relative">
                        <Link href={`/photo-albums/${album}`}>
                            <Image priority className="object-cover w-60 h-80 hover:blur-sm rounded-xl hover:object-scale-down hover:bg-neutral-900" src={photos[album].photos[0].url} width={500} height={500} alt={photos[album].photos[0].name} />
                        </Link>
