...
async function getSponsorLogos() {
    const file = await fs.readFile(process.cwd() + '/public/assets/sponsor_logos/sponsors.json', 'utf-8');
    const data = JSON.parse(file);
    return data;
}
const SponsorsLayout = async () => {
                    ...
                    <div key={index} className="flex overflow-hidden min-w-max">
                        {index % 2 === 0 && (
                            ...
                            <div className="shrink-0 h-28 w-80 grow bg-cover flex justify-center items-center overflow-hidden" style={{ backgroundImage: "url(/assets/sponsor_bkgs/standardbkg.jpeg)" }}>
                                <Link target="_blank" href={sponsors[index][1]} rel="noopener noreferrer">
                                    <img className="w-60 max-w-fit max-h-28" src={sponsors[index][0]} alt={sponsors[index][2]}/>
                                </Link>
                            </div>
                            <div className="shrink-0 h-28 w-80 grow bg-cover flex justify-center items-center overflow-hidden" style={{ backgroundImage: "url(/assets/sponsor_bkgs/standardbkg.jpeg)" }}>
                                <Link target="_blank" href={sponsors[index+1][1]} rel="noopener noreferrer">
                                    <img className="w-60 max-w-fit max-h-28" src={sponsors[index+1][0]} alt={sponsors[index+1][2]}/>
                                </Link>
                            </div>
                            </>
                        )}
